...
public class TuyaLanDiscoveryService(
    ...
    : BackgroundService
{
    private const int TuyaPort = 6667;
    private const uint Prefix55AA = 0x000055AA;
    private const int HeaderSize = 16;
    private const int RetcodeSize = 4;
    private const int EndSize = 8;

    // MD5("yGAdlopoPVldABfn") — nonce hardcoded do protocolo Tuya LAN
    private static readonly byte[] NonceKey =
        MD5.HashData(Encoding.UTF8.GetBytes("yGAdlopoPVldABfn"));
    ...
    {
        logger.LogInformation("TuyaLanDiscoveryService iniciado — escutando UDP:{Port}", TuyaPort);

        using var udp = new UdpClient();
        udp.Client.SetSocketOption(SocketOptionLevel.Socket, SocketOptionName.ReuseAddress, true);
        udp.EnableBroadcast = true;
        udp.Client.Bind(new IPEndPoint(IPAddress.Any, TuyaPort));
    ...
    {
        if (data.Length < HeaderSize + RetcodeSize + EndSize) return null;

        var prefix = ReadUInt32BE(data, 0);
        if (prefix != Prefix55AA) return null;

        var length        = ReadUInt32BE(data, 12);
        var expectedTotal = HeaderSize + (int)length;
        if (data.Length < expectedTotal) return null;

        var ciphertext = data[(HeaderSize + RetcodeSize)..(expectedTotal - EndSize)];
